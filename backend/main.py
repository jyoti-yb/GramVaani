...
async def get_crop_calendar(current_user: dict = Depends(get_current_user)):
    """Get seasonal crop calendar from MongoDB (702 districts, 3510+ data points)"""
    try:
        location = current_user.get('location', '')
        print(f"\n=== CROP CALENDAR REQUEST ===")
        print(f"User location: {location}")
        
        parts = [p.strip() for p in location.split(',')]
        district = parts[0] if parts else 'Unknown'
        ...
        if api_key:
            try:
                ...
                if res.status_code == 200:
                    ...
                    print(f"Weather: {weather_data['main']['temp']}°C, {weather_data['main']['humidity']}%")
            except Exception as e:
                print(f"Weather error: {e}")
        
        # Search MongoDB (702 districts coverage)
        hyperlocal_data = None
        try:
            print(f"Searching MongoDB for: {district}")
            
            # Try exact district match first
            hyperlocal_data = hyperlocal_collection.find_one(
                {"district": {"$regex": f"^{district}$", "$options": "i"}}
            )
            
            if not hyperlocal_data:
                # Try partial district match
                hyperlocal_data = hyperlocal_collection.find_one(
                    {"district": {"$regex": district, "$options": "i"}}
                )
            
            if not hyperlocal_data and state:
                # Try state + district combination
                hyperlocal_data = hyperlocal_collection.find_one({
                    "$and": [
            ...
            if not hyperlocal_data and state:
                # Get any district from the state
                hyperlocal_data = hyperlocal_collection.find_one(
                    {"state": {"$regex": state, "$options": "i"}}
                )
            ...
            if not hyperlocal_data:
                ...
                nearby = ['Guntur', 'Vijayawada', 'Krishna', 'Visakhapatnam', 'Tirupati']
                for city in nearby:
                    hyperlocal_data = hyperlocal_collection.find_one(
                        {"district": {"$regex": city, "$options": "i"}}
                    )
                    if hyperlocal_data:
                        print(f"Using nearby district: {city}")
                        break
            
            if hyperlocal_data:
                print(f"\n✅ Found MongoDB data:")
                print(f"   District: {hyperlocal_data.get('district')}")
                print(f"   State: {hyperlocal_data.get('state')}")
                print(f"   Soil: {hyperlocal_data.get('soil_type')}")
                print(f"   Rainfall: {hyperlocal_data.get('rainfall')}")
                print(f"   Crops: {hyperlocal_data.get('crops', {})}")
        except Exception as e:
        ...
        if not hyperlocal_data:
            ...
            raise HTTPException(status_code=404, detail=f"No agricultural data found for {district}. Please update your location.")
        
        # Get crops for current season
        crops_data = hyperlocal_data.get('crops', {})
        season_crops = crops_data.get(season, [])
        
        if not season_crops:
            print(f"\n⚠️ No {season} crops found, checking all seasons...")
            # Try other seasons as fallback
            for s in ['kharif', 'rabi', 'summer']:
                season_crops = crops_data.get(s, [])
                if season_crops:
                    print(f"Using {s} season crops as fallback")
                    break
        
        if not season_crops:
            raise HTTPException(status_code=404, detail=f"No crop data available for {season} season in {hyperlocal_data.get('district')}")
        
        print(f"\n🌾 Crops for {season}: {season_crops}")
        
        # Build crop recommendations
        recommended_crops = []
        for crop_name in season_crops:
            if season == 'kharif':
                planting_start, planting_end = 'June', 'July'
                harvest_start, harvest_end = 'October', 'November'
                duration = 120
            elif season == 'rabi':
                planting_start, planting_end = 'November', 'December'
                harvest_start, harvest_end = 'March', 'April'
                duration = 120
            else:
                planting_start, planting_end = 'February', 'March'
                harvest_start, harvest_end = 'May', 'June'
                duration = 90
            ...
            crop_data = {
                ...
                'season': season,
                'recommended': True,
                'reason': f"Recommended for {season} season in {hyperlocal_data.get('district')}",
                'soil_type': hyperlocal_data.get('soil_type', 'Various'),
                'rainfall': hyperlocal_data.get('rainfall', 'Moderate'),
                'planting': {'start': planting_start, 'end': planting_end},
                'harvesting': {'start': harvest_start, 'end': harvest_end},
            ...
            if weather_data:
                ...
                if temp > 35:
                    crop_data['weather_tip'] = '🌡️ Very hot - ensure irrigation, consider delaying planting'
                elif temp > 30:
                    crop_data['weather_tip'] = '🌡️ High temperature - ensure adequate irrigation'
                elif temp < 15:
                    crop_data['weather_tip'] = '❄️ Cool weather - protect seedlings from frost'
                elif temp < 10:
                    crop_data['weather_tip'] = '❄️ Very cold - not suitable for planting'
                
                if humidity > 80:
                    crop_data['weather_tip'] = crop_data.get('weather_tip', '') + ' 💧 High humidity - monitor for fungal diseases'
                elif humidity < 30:
                    crop_data['weather_tip'] = crop_data.get('weather_tip', '') + ' 💧 Low humidity - increase irrigation frequency'
        ...
        result = {
            ...
            'user_location': f"{hyperlocal_data.get('district')}, {hyperlocal_data.get('state')}",
            ...
            'data_source': 'MongoDB - 702 districts coverage'
        }
        
        print(f"\n✅ Returning {len(recommended_crops)} crops")
        return result
        
    except HTTPException:
        raise
    except Exception as e:
        print(f"\n❌ Crop calendar error: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
